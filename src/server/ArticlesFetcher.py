...
class ArticlesFetcher:
    ...
    def fetch_recent(self, labels, exclude, skip=0):
        ...
        db_articles: list[Article] = Article.query.filter(Article.rss.in_(exclude)).order_by(desc(Article.pub_date)).all()
    ...
    def fetch_popular(self, labels, exclude, skip=0):
        ...
        db_articles = Article.query.filter(cast(Article.pub_date, Date) >= seven_days_ago.date(), Article.rss.in_(exclude)).order_by(desc(Article.views)).all()
    ...
    def fetch_recommended(self, labels, user_id, exclude, skip=0):
        ...
        all_articles = Article.query.filter(Article.rss.in_(exclude)).all()
